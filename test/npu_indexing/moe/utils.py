...
def old_get_capacity(gates: Tensor, capacity_factor: Tensor, min_capacity: Tensor) -> Tensor:
...
def get_capacity(gates, capacity_factor, min_capacity):
    # gates has shape of S,E
    num_tokens = gates.shape[0]
    num_experts = gates.shape[1]
    # to(torch.int64) works around a bug in torch.onnx.export:
    # it should cast k to int64 when converting torch.topk but it doesn't.
    key = (num_tokens, num_experts, min_capacity)
    if key not in capacity_map:
        capacity = _capacity(num_tokens, num_experts, capacity_factor, min_capacity)
        capacity_map[key] = (torch.tensor(capacity, device=gates.device, dtype=torch.int32), capacity)
