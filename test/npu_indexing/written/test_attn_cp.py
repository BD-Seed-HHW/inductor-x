...
from test2.npu_indexing.written.attn_cp_triton_kernel_3d import forward_update_triton, backward_update_triton
from test2.npu_indexing.written.attn_cp_triton_kernel_3d_la import forward_update_triton as forward_update_triton_la
...
def benchmark_test(fn, fn_triton, args =(), name="gen_fn", times=10, repeat=10, profile=False):
    ...
    print(f"Accelerated: {(time_eager - time_compiled)/time_compiled*100:.4f}% eager takes {time_eager:.3f} us, triton takes {time_compiled:.3f} us")
...
def prove_forward_update():
    ...
    def data_validation(prev_softmax_max, cur_softmax_max, prev_softmax_sum, cur_softmax_sum , prev_attn_out, cur_attn_out):
        ...
        (attn_out, softmax_max, softmax_sum) = forward_update(prev_attn_out, prev_softmax_max, prev_softmax_sum, cur_attn_out,
    ...
    for i in range(1) :
        ...
        data_validation(prev_softmax_max, cur_softmax_max, prev_softmax_sum, cur_softmax_sum , prev_attn_out, cur_attn_out)

        print("--------------------prove_forward_update:1S---------------------- ")
        data_validation(prev_softmax_max_s, cur_softmax_max_s, prev_softmax_sum_s, cur_softmax_sum_s , prev_attn_out_s, cur_attn_out_s)
...
def benchmark_forward_update() :
    ...
    benchmark_test(forward_update, forward_update_triton, args=(prev_attn_out,
                prev_softmax_max, prev_softmax_sum, cur_attn_out, cur_softmax_max, cur_softmax_sum),name="forward_update_2s",  times=10, repeat=10)

    # benchmark_test(forward_update, forward_update_triton, args= (prev_attn_out,
    #             prev_softmax_max, prev_softmax_sum, cur_attn_out, cur_softmax_max, cur_softmax_sum),name="forward_update_s",  times=10, repeat=10)
...
def forward_update_la(prev_attn_out, prev_softmax_log_max_sum,
    ...
    attn_out = torch.exp(prev_softmax_log_max_sum - softmax_log_max_sum) * prev_attn_out + torch.exp(cur_softmax_log_max_sum - softmax_log_max_sum) * cur_attn_out
...
def benchmark_forward_update_la():
     ...
     benchmark_test(forward_update_copy, forward_update_triton_la, args=(attn_out_s[:,:,1], softmax_sum_s[:,:,1],
         cur_attn_out_s, cur_softmax_sum_s), name="forward_update_la", profile=False, repeat = 1000)
...
if  __name__ == "__main__":
     ...
     benchmark_forward_update()
#     benchmark_forward_update_la()
#     benchmark_backward_update()
 #   profile_forward_update()
